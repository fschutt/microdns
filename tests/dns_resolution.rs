use microdns::{lookup_mx_records, lookup_ip_addresses, DnsConfig, Error};
use std::net::IpAddr;
...
fn test_google_mx_records() -> Result<(), Error> {
    ...
    let google_mx = mx_records.iter().find(|mx| mx.server.contains("google.com"));
    assert!(google_mx.is_some(), "No Google mail servers found in MX records");
...
fn test_mx_server_ip_resolution() -> Result<(), Error> {
    ...
    assert!(!ips.is_empty(), "No IP addresses found for MX server {}", first_mx.server);
